// WG Created: 24/11/22 Modified: 24/11/22
// from "third_test_loop.ino"
...
void loop() {
  ...
  if(position == 999){ // Wait for button press before starting
    ...
    if(WideLeft.is_White() && (currentMillis - previousMillis > 0.8 * expectedSectionDuration) && loop_counter == 0){ //for junction turns as well.
      delay(300);
      turn_left_90();
      while ((Left.is_White()) || (Right.is_White())){
        follow_line();
      }
      turn_right_90();
      turn_right_90();
  ...
  expectedSectionDuration = 38/robotSpeed;
    if((WideRight.is_White() || WideLeft.is_White()) && (currentMillis - previousMillis > 0.6 * expectedSectionDuration) && loop_counter == 0){ //
  ...
  expectedSectionDuration = 38/robotSpeed;
    if ((WideLeft.is_White() && currentMillis - previousMillis > 0.6 * expectedSectionDuration) && (loop_counter == 0)){
      delay(300);
      turn_left_90();
      follow_line();
      delay(1200);
